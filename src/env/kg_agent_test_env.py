...
class KGAgentTestEnv:
    def __init__(
        ...
        **kwargs
        ...
        self.model = model
        self.agent = get_agent("kg_agent", f"{project_root}/src/agents/prompts/kg_agent_prompt.txt", model=model)
...
if __name__ == "__main__":
    ...
    while not done:
        state, done = env.step()
    env.save_state(f"data/prompt_engineering/kg_agent/kg_agent_test_history_{time.strftime('%Y%m%d_%H%M%S')}.json")