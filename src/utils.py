...
def get_completion(model: str, messages: list, temperature: float = 1.0, max_retries=3, **kwargs):
    for attempt in range(1, max_retries+1):
        try:
            ...
            assert response is not None, "Invalid response from completion API: None"
            assert response.choices and len(response.choices) > 0, "Invalid response from completion API: No choices"
            return response
